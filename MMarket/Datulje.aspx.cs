...
namespace MMarket
{
    public partial class Datulje : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string datulje = "datulje";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @datulje", con);
            com.Parameters.AddWithValue("@datulje", datulje);

            con.Open();

            SqlDataAdapter adptr = new SqlDataAdapter(com);

            ViewState["datulje"] = dt;
        ...
        private void AddToCart_ServerClick(object sender, EventArgs e)
        {
            if (((HtmlButton)sender).ID.First() == 'd')
            {
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["datulje"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["datulje"]).Rows[row]);
                    }
                }
            }
            else
