...
namespace MMarket
{
    public partial class CajeviKave : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string kava = "cajevi i kave";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @kava", con);
            com.Parameters.AddWithValue("@kava", kava);

            con.Open();

            SqlDataAdapter adptr = new SqlDataAdapter(com);

            ViewState["CajKava"] = dt;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                // tu skopiraš style i tako na svakoj stranici
        ...
        {
            if (((HtmlButton)sender).ID.First() == 'C')
            {
                if (Session["CartTable"] == null)
                {
                    DataTable dt = new DataTable();

                    dt = ((DataTable)ViewState["CajKava"]).Clone();

                    string resultString = Regex.Match(((HtmlButton)sender).ID, @"\d+").Value;
                    int row = int.Parse(resultString);
                    dt.ImportRow(((DataTable)ViewState["CajKava"]).Rows[row]);
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["CajKava"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["CajKava"]).Rows[row]);
                    }
                }
            }
            else
