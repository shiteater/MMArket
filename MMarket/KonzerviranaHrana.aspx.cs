...
namespace MMarket
{
    public partial class KonzerviranaHrana : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string konzerve = "konzerve";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @konzerve", con);
            com.Parameters.AddWithValue("@konzerve", konzerve);

            con.Open();

            SqlDataAdapter adptr = new SqlDataAdapter(com);

            ViewState["konzerve"] = dt;
        ...
        private void AddToCart_ServerClick(object sender, EventArgs e)
        {
            if (((HtmlButton)sender).ID.First() == 'k')
            {
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["konzerve"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["konzerve"]).Rows[row]);
                    }
                }
            }
            else
