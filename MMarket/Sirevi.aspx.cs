...
namespace MMarket
{
    public partial class Sirevi : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string sirevi = "sirevi";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @sirevi", con);
            com.Parameters.AddWithValue("@sirevi", sirevi);
        ...
        {
            if (((HtmlButton)sender).ID.First() == 's')
            {
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["sirevi"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["sirevi"]).Rows[row]);
                    }
                }
            }
            else
