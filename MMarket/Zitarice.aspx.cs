...
namespace MMarket
{
    public partial class Zitarice : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string zitarice = "zitarice";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @zitarice", con);
            com.Parameters.AddWithValue("@zitarice", zitarice);

            con.Open();

            SqlDataAdapter adptr = new SqlDataAdapter(com);

            ViewState["zitarice"] = dt;
        ...
        {
            if (((HtmlButton)sender).ID.First() == 'z')
            {
                if (Session["CartTable"] == null)
                {
                    DataTable dt = new DataTable();

                    dt = ((DataTable)ViewState["zitarice"]).Clone();

                    string resultString = Regex.Match(((HtmlButton)sender).ID, @"\d+").Value;
                    int row = int.Parse(resultString);
                    dt.ImportRow(((DataTable)ViewState["zitarice"]).Rows[row]);
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["zitarice"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["zitarice"]).Rows[row]);
                    }
                }
            }
            else
