...
namespace MMarket
{
    public partial class Ostalo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string ostalo = "ostalo";
            SqlCommand com = new SqlCommand("SELECT [idProizvod], [Naziv], [Opis], [Cijena], [NazFile], [Tezina], [Countity] FROM [Proizvodi] WHERE [Kategorija] LIKE @ostalo", con);
            com.Parameters.AddWithValue("@ostalo", ostalo);
        ...
        private void AddToCart_ServerClick(object sender, EventArgs e)
        {
            if (((HtmlButton)sender).ID.First() == 'o')
            {
                ...
                else
                {
                    ...
                    {
                        if ((int)item.ItemArray[0] == (int)((DataTable)ViewState["ostalo"]).Rows[row].ItemArray[0])
                        {
                            dodaj = false;
                        }
                    }
                    ...
                    if (dodaj)
                    {
                        ((DataTable)Session["CartTable"]).ImportRow(((DataTable)ViewState["ostalo"]).Rows[row]);
                    }
                }
            }
            else
